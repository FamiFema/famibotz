...
let handler = async (m, { conn, usedPrefix }) => {
  ...
  if (id in conn.tebakbendera) {
    ...
    throw false
      ...
      let src = await (await fetch('https://raw.githubusercontent.com/Aal-oss/Akazamdv11/main/scraper/bendera.json')).json();
  let json = src[Math.floor(Math.random() * src.length)]
  let caption = `
  Bendera apakah ini?
Timeout *${(timeout / 1000).toFixed(2)} detik*
Ketik ${usedPrefix}tebe untuk bantuan
mau nyerah? ${usedPrefix}nyerahben untuk nyerah
Bonus: ${poin} XP
    `.trim()
