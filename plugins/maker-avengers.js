...
let handler = async (m, { conn, text }) => {
  ...
  try {
    let res = await fetch(`https://api.akuari.my.id/textpro/scraper-2?text=${encodeURIComponent(text1)}&text2=${encodeURIComponent(text2)}&link=https://textpro.me/create-3d-avengers-logo-online-974.html`);
    let result = await res.json();

    if (result.respon) {
      conn.sendMessage(m.chat, {
        image: { url: result.respon },
