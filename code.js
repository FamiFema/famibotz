process.env['NODE_TLS_REJECT_UNAUTHORIZED'] = '0'; //0 ganti sesuai selera
//terserah mau pake apaan kalo mau pake diatas tinggal hapus tanda//
import './config.js'
...
if(usePairingCode && !conn.authState.creds.registered) {
		if(useMobile) console.log('Gk bisa nautkan kode dengan mobile api')
		const { registration } = { registration: {} }
		let phoneNumber = ''
		do {
			phoneNumber = await question(chalk.blueBright('masukan nomor yang valid untuk menautkan contoh 628xxxx\n'))
		} while (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v)))
		rl.close()
		phoneNumber = phoneNumber.replace(/\D/g,'')
		console.log(chalk.bgWhite(chalk.blue('Mengambil kode please wait bro')))
		setTimeout(async () => {
			let code = await conn.requestPairingCode(phoneNumber)
			code = code?.match(/.{1,4}/g)?.join('-') || code
			console.log(chalk.black(chalk.bgGreen(`ni kode : `)), chalk.black(chalk.white(code)))
		}, 3000)
	}
