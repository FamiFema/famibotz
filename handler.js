        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    ...
                    this.reply(m.chat, `[❗] Limit kau abis dek, kontak owner untuk membeli atau claim limit daily kamu melalui *${usedPrefix}daily*`, m)
        ...
        case 'add':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    let ppgp = './src/avatar_contact.png';
                    try {
                        ...
                        let welcomeApiUrl = `https://api.akuari.my.id/canvas/welcome?name=${encodeURIComponent(name)}&gcname=${encodeURIComponent(gpname)}&ppgc=${encodeURIComponent(ppgp)}&member=${encodeURIComponent(nthMember.toString())}&pp=${encodeURIComponent(pp)}&bg=${encodeURIComponent('https://i.ibb.co/tYgwwT2/images-2.jpg')}`;
                        try {
                            let welcomeResponse = await fetch(welcomeApiUrl);
                            let welcomeBuffer = await welcomeResponse.buffer();

                            await this.sendMessage(id, {
                                image: { url: welcomeApiUrl },
                                caption: text
                            });
            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        let name = await this.getName(user);
                        let gpname = await this.getName(id);
                        // Use the provided URL for the goodbye action
                        let leaveApiUrl = `https://api.akuari.my.id/canvas/goodbye?name=${encodeURIComponent(name)}&gcname=${encodeURIComponent(gpname)}&ppgc=${encodeURIComponent(ppgp)}&member=${encodeURIComponent(nthMember.toString())}&pp=${encodeURIComponent(pp)}&bg=${encodeURIComponent('https://i.ibb.co/tYgwwT2/images-2.jpg')}`;
                        try {
                            let leaveResponse = await fetch(leaveApiUrl);
                            let leaveBuffer = await leaveResponse.buffer();

                            await this.sendMessage(id, {
                                image: { url: leaveApiUrl },
                                caption: text
                            });
                        } catch (error) {
...
let msg = {
       ...
       botAdmin: 'Jadikan aku Sebagai Admin Terlebih Dahulu Agar Bisa Menggunakan Fitur Ini',
